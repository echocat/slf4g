...
func Test_Provider_MustContains(t *testing.T) {
	...
	expectedEvent1 := log.NewEvent(instance, level.Info, 2).
		With("message", "a")
	expectedEvent2 := log.NewEvent(instance, level.Warn, 2).
		With("message", "b").
		With("foo", 1)
	unExpectedEvent3 := log.NewEvent(instance, level.Info, 2).
		With("message", "b").
		With("foo", 1)
...
func Test_Provider_MustContains_panicsOnErrorsForRoot(t *testing.T) {
	...
	givenEvent := log.NewEvent(instance, level.Info, 2).
...
func Test_Provider_GetAll(t *testing.T) {
	...
	assert.ToBeEqualUsing(t, log.NewEvent(instance, level.Info, 2).
		With("message", "a").
		With("foo", 1).
		With("bar", 2), actual[0], instance.defaultEventEquality().AreEventsEqual)
	assert.ToBeEqualUsing(t, log.NewEvent(instance, level.Warn, 2).
		With("message", "b").
		With("foo", 1).
...
func Test_Provider_GetOf(t *testing.T) {
	...
	assert.ToBeEqualUsing(t, log.NewEvent(instance, level.Error, 2).
		With("message", "c").
		With("foo", 1).
