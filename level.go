...
// important is the event. LevelTrace is the less severe and LevelFatal the
// most severe.
//
// Customization
//
// Different implementations of Provider could introduce more Levels. All
// ordinals are unique over all instances of Level. LevelInfo = 3000 will be
// always LevelInfo = 3000. Another Level which uses the ordinal 3000 can be
// just assumed as an alias to LevelInfo.  Customization only means added new
...
// DefaultLevelProvider is the default instance of a LevelProvider.
// this could be overwritten for customization purposes.
var DefaultLevelProvider = defaultLevelProvider

// DefaultLevelProviderFacade always uses the latest reference of
// DefaultLevelProvider regardless when DefaultLevelProvider was set. This is
// very useful while initiating Provider and Logger globally where it is hard to
// guarantees the order how each of these init() methods across the whole
// project and all of its libraries is called.
...
var defaultLevelProvider LevelProvider = func() Levels {
	return Levels{LevelTrace, LevelDebug, LevelInfo, LevelWarn, LevelError, LevelFatal}
}
